...
function updateFilters() {
  ...
  function filterTable() {
    ...
    // filters.forEach((filtersRow) => {
    //   let id = Object.keys(filtersRow);
    //   let val = Object.values(filtersRow);
    //   filteredData = filteredData.filter(row => row.id === val)});
  
    for (filter in filters) {
      filteredData = filteredData.filter(row => row[filter] === filters[filter])
      console.log(filters[filter])
      // let newRow = 'row.'+Object.keys(filters);
      // let val = Object.values(filters);
      // filteredData = filteredData.filter(row => newRow[i] === val[i]);
